...
| 6. Cost and runtime | in progress | R1-R4 | Cumulative $1.50 control-plane policy, exact Secure 2xA40 40 GB ephemeral request with zero persistent volume, create-rate deadline, malformed-response reconciliation, delayed billing receipts, Torch-preserving frozen RunPod requirements, fail-fast aggregate preflight, conservative paired runtime evidence, output-equivalence tests, and dynamic CUDA validation | Under the current explicit paid-run instruction, run R4 and only the one-step smoke if R4 passes and settled spend leaves budget |
