...
| 3. Canonical RunEngine | partial | F9-F11, F14 | One fail-closed config dispatcher, shared stage order, central manifest and promotion, one package console script, and removal of shadow task inference commands | Resolve U-038 and U-071 by moving consequential preparation and runtime certification behind engine stages |
...
| 9. Final compliance and fresh-eyes review | in progress | All criteria | Apache-2.0, architecture contracts, locked tests, Ruff, build, secret scanning, and the adversarial risk register | Resolve U-038 and U-071; preserve honest external blockers and collect remote certification evidence only under explicit authorization |
