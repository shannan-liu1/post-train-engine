...
| U-061 | Deterministic BF16 generation produced different scalar and batched completions on the Secure two-A40 topology. | A large measured speedup could hide a semantic runtime change and certify a false optimization. | Keep exact-output parity fail closed, retain the failed artifact, and diagnose batch-shape numerical sensitivity before another paid attempt. | R4 failed remotely; diagnosis open |
...
| U-065 | The successful local provider watchdog was a task-local ignored script rather than a tested repository command. | A later operator could hand-roll another safety-critical process and repeat quoting, credential, or lifecycle mistakes. | Add one canonical detached local watchdog launcher with public-interface tests before another paid Pod. | open; blocks next paid attempt |
...
| 6. Cost and runtime | in progress | R1-R4 | Secure 2xA40 remote preflight and topology passed on commit `01bb8ba`; R4 measured conservative speedup 1.973 but failed exact output parity; GRPO did not run; provider billing remains pending; future failures retain per-trial parity and deletion journals close honestly | Add the canonical local watchdog launcher, diagnose scalar-versus-batched BF16 generation drift, and settle the delayed provider receipt before requesting a second paid Pod |
...
| 9. Final compliance and fresh-eyes review | in progress | All criteria | Apache-2.0, 307 locked tests, Ruff, build, secret scanning, two post-change fresh-eyes passes, and the adversarial risk register | Preserve honest external blockers and collect remote certification evidence only under explicit authorization |
