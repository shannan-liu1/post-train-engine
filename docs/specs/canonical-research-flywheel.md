...
| 6. Cost and runtime | in progress | R1-R4 | Secure 2xA40 remote preflight and topology passed on commit `01bb8ba`; R4 measured conservative speedup 1.973 but failed exact output parity; GRPO did not run; provider billing remains pending; failures retain per-trial parity; deletion journals close honestly; the canonical local watchdog uses the absolute create deadline with bounded deletion retries; benchmark v3 isolates model reuse from batching and now runs through RunEngine | Settle the delayed provider receipt and collect canonical remote R4 evidence before any GRPO smoke |
