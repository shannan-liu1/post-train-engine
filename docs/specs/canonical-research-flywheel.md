...
| U-071 | The paid R4 benchmark remains a standalone verification script rather than a RunEngine Run. | Paid evidence can remain outside the canonical RunManifest and phase-cost lifecycle even though it cannot promote or train. | Keep the benchmark non-promoting and fail closed, but move runtime certification into typed RunEngine stages before claiming one complete canonical path or running another paid certification attempt. | open; canonical path blocker |
...
| 3. Canonical RunEngine | partial | F9-F11, F14 | One fail-closed config dispatcher, engine-owned typed resolution, shared stage order, central manifest and promotion, one package console script, and removal of shadow task inference commands | Resolve U-071 by moving runtime certification behind engine stages |
...
| 9. Final compliance and fresh-eyes review | in progress | All criteria | Apache-2.0, architecture contracts, locked tests, Ruff, build, secret scanning, and the adversarial risk register | Resolve U-071; preserve honest external blockers and collect remote certification evidence only under explicit authorization |
