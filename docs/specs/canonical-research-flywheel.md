...
| U-114 | RunPod's Cloudflare policy rejected Python urllib's default browser signature with HTTP 403 Error 1010, while the create layer discarded the bounded response and labeled the definitive rejection ambiguous. | No Pod was created, but the operator received a misleading failure and the watchdog retained unnecessary ambiguous supervision. | Send one canonical application User-Agent on RunPod requests, preserve redacted bounded 4xx detail, record definitive create rejection, and close its watchdog after provider absence. | implemented after paid create rejection |
