...
| U-029 | Promotion compares candidate-produced artifacts without binding them to one immutable suite, prompt, verifier, generation, and RunPlan contract. | A candidate or adapter can change the evaluation law and still satisfy numeric gates. | Add a signed or content-addressed EvalContract to RunPlan; require both paired artifacts to match it before `decide_promotion`. | open, promotion foundation |
| U-030 | Final Run bundles expose per-example promotion rows and failure annotations. | Researchers or agents can adapt to sealed holdout content despite split separation. | Store sealed rows behind a restricted evidence boundary and expose aggregate promotion results to the normal report and agent surfaces. | open, promotion foundation |
| U-031 | Content and verifier separation checks exist as isolated helpers and tests, while replay records lack canonical SplitRole and policy lineage. | Training can bypass leakage checks through a non-TrainingView evidence path. | Integrate separation certificates into RunEngine promotion and replace raw replay writes with role-aware Trace queries. | open, evidence foundation |
| U-032 | Stage receipts reference mutable paths rather than immutable output hashes, and provider side effects can complete before receipt durability. | Resume can accept changed outputs or duplicate paid work. | Content-address receipt outputs and use transactional provider idempotency keys with reconciliation tests. | open, RunEngine foundation |
...
| U-037 | The project has no lockfile, dependencies float, and Muon resolves from a mutable Git branch. | A commit does not reproduce its Python or optimizer environment. | Commit a cross-platform lock and pin Git dependencies to reviewed immutable commits before production certification. | open, reproducibility |
...
| U-043 | Local private-repository access did not prove that the execution boundary permitted sending a PAT to a third-party Pod. | A paid Pod can pass hardware gates but remain unable to receive its exact source commit. | Require an end-to-end source-authentication preflight; use a repository-scoped read-only deploy key through SSH agent forwarding when PAT transport is prohibited. | open, owner must register deploy public key |
...
| 1. Portable RunManifest and RunBundle | partial | F1-F4, F8, F14 | Canonical writer, relocation and source tests, immutable Hub revision resolver, atomic manifest finalization, ArtifactStore ownership marker, lifecycle deletion containment, and TrainingView consumption hashes | Complete source-state coverage, immutable receipt outputs, and one containment reader |
| 2. Sealed evaluation and strict promotion | partial | F5-F8 | EvaluationRoles, one PromotionDecision, and isolated content and verifier safety tests | Bind promotion to an immutable EvalContract and restrict sealed row disclosure |
...
| 6. Cost and runtime | in progress | R1-R4 | Explicit missing-cost state, efficiency summary, cache primitives, output-equivalence tests, dynamic CUDA-image validation, exact hardware attestation, ephemeral smoke storage, and a bounded attempt that verified 2x A40 with Torch CUDA 12.8 before source authentication failed closed | Register a read-only deploy key, prove its transport without sending private material to the Pod, then rerun R4 from the exact pushed commit |
