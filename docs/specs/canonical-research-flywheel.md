...
| 6. Cost and runtime | in progress | R1-R4 | Secure 2xA40 remote preflight and topology passed on commit `01bb8ba`; R4 measured conservative speedup 1.973 but failed exact output parity; GRPO did not run; provider billing settled at `$0.12157369661144912`; cumulative campaign spend is `$0.5037198807985988`; the local attempt now enforces a receipt-anchored 20-minute ceiling across resumes, provider-scheduled termination, pre-create name watchdog, zero active Pods, exact public Git source, immutable plan identity, a digest-pinned image, hash-checked remote dependencies, terminal-status polling, phase reserves, R4-first execution, a 256 MiB evidence ceiling, pinned SSH resume, journal identity checks, verified deletion, two-observation settlement, and a reloaded persistent SSH identity whose private half remains outside the workspace | Under explicit paid authorization, collect canonical remote R4 evidence before any GRPO smoke |
